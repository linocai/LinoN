...
- **规则单一源不漂移**:选股硬规则(黑名单按**板块前缀**:创业板 `30*`/科创 `688*`+`689*`/北交所 `8*`+`4*`+`920*`、ST、白酒行业)全在 `app/screen/rules.py` 顶部;`-5.0/+15/D4/容差带` 仍只在 `app/db/store/constants.py`(选股层不碰)。**代码段黑名单坑(2026-06-28 三次同类漏后统一修)**:枚举精确段易随交易所新增子段漏挡,**已统一收为板块整段正则 `^(30|688|689|8|4|920)`**——创业板旧 `300` 漏 301/302(信濠光电 301051)、科创旧 `688` 漏 689 CDR(九号 689009)、旧无 `920` 漏北交所新段(莱赛激光 920363)。**教训:黑名单按板块整段、勿枚举精确子段**。**铁律:技术面交 LLM 判**——`rules.py` 的宽筛阈(`VOL_RATIO_MIN=1.5`/`NEW_HIGH_DAYS=20`/`MA_DAYS=20`/近3日净流入>0)都标注"宁松勿紧、复盘迭代、不卡生死",不当死阈值。**v1.3.1 起(2026-07-05)高位线 ≥100% 不再硬排除,改只标注 warnLevel(≥100% 红 `high`/[50,100%) 琥珀 `amber`,可与单日暴涨叠加);粗筛/排序放量口径从自算放量倍数换成官方 `daily_basic.volume_ratio`(展示侧 `volMultiple`/`volPct` 仍自算,展示与排序口径解耦);排序权重从 4 键(`vol/fund/turnover/low`)扩为 9 因子(量比0.30/位置健康`pos_health`距60日高点0.16/换手健康[7,15]%0.14/VWAP0.10/横盘突破0.10/市值弹性[50,500]亿0.08/活跃0.06/资金0.06/单日软闸-0.06),`pos_health` 替换旧 `low_position`(旧版偏好左侧下跌票、方向反了)。选股规则从此可经 `screen_config` 表运行时覆盖(`rules.py` 常量降为默认/fallback,深判层 `analyze.py` 不吃用户配置)。详见 `archive/v1.3.1_plan.md`。
- **Tushare 真实字段口径(2026-06 冒烟校验,务必照此解析)**:全市场批量接口不带 `ts_code`、只带 `trade_date` → 一次返回全市场。`daily_basic`:`close`/`turnover_rate`(%)/`total_mv`(**万元**,÷1e4→亿)/`volume_ratio`(量比,本期未用)。`moneyflow_dc`(**选股资金面唯一信号,东财源,见下条**):`net_amount`(**万元**,主力净额)。`daily`:`vol`(手)/`amount`(**千元**)/`close`/`pre_close`/`pct_chg`(%)。放量倍数=当日 vol/前5日均 vol(自己算,非 volume_ratio)。`moneyflow`(原始源,**深析层已弃用**——2026-07-02 深析层也切东财 `moneyflow_dc`;`net_mf_amount` 与东财 `net_amount` **口径不同、能符号相反**,曾导致深析卡显示与候选列表/同花顺相反的净流出)。
...
- **资金时序(2026-07-02 修,推翻旧"一律取上一交易日"写死)**:深析层资金源 = 东财 `moneyflow_dc`(`net_amount`,6000 积分,与选股层/用户同花顺方向一致)。`fund_asof` 取 `_fetch_fund` 返回的 **`asof` = 实际拿到的资金最新交易日**(盘后=今日 EOD、盘中=上一交易日,因 `moneyflow_dc` 盘中只给到 T-1);仅拉取失败无数据时才退回 `fund_asof_date()`(严格上一交易日)占位。**旧写死 `prev_trading_day` 会在盘后把今天数据误标成昨天**(实测 07-02 盘后把 07-02 的数据标成 07-01)。客户端文案改"资金面 = 截至 {date} EOD · 东财主力口径(非盘中实时)"。`_fetch_fund` 读列 `net_amount`(东财)|回退 `net_mf_amount`(老源,供注入测试)。
...
- **候选刷新已改纯手动(v1.3.1,2026-07-05 起,订正旧 15:35 自动 tick 描述)**:`loop.py` 的 15:35 自动 candidate refresh tick、`_is_after_candidate_window`/`_CANDIDATE_AFTER`、`last_candidate_date`、编排函数 `run_candidate_refresh` **均已删除**(`run_candidate_refresh` 从不被 `POST /candidates/refresh` 端点调用,是死码)。**`POST /candidates/refresh`(走 `app.py._recompute_candidates`)是唯一刷新入口**,只能用户手动触发。候选回测回填(阶段2.5 F3)改挂进 `last_eod_date` 守卫的 EOD 摘要块内(每交易日一次,不再依赖已删的 candidate tick)。EOD 持仓摘要推送本身不受影响。
...
- **坑6 ECS→DeepSeek(EdgeOne CDN)偶发单连接卡死(2026-07-02)**:`/analyze`、`/coach` 深析报"网络错误"→ 排查链:① 客户端 `post` 默认超时 12s < 后端 DeepSeek 同步调用耗时(先修客户端 analyze/coach 超时 12→60s,APIClient.swift);② 之后露出后端"调用异常 ReadTimeout"降级卡。**根因不是资源/MTU/DeepSeek 慢**:ECS 内存/swap/负载全闲;`ping -M do` 1500B 全通(非 MTU);DeepSeek 从本机 Mac ~3-4s、从 ECS 连打 8 次全 <1s——**平时健康,偶发单个连接读响应体卡死到超时**(api.deepseek.com 走腾讯 EdgeOne CDN `eo.dnse1.com`,`connect=0.009s`/`TTFB=0.2s` 秒回但 `total` 打满超时、空体)。**修 `deepseek.py`:短读超时(`_READ_TIMEOUT=12`/`_CONNECT_TIMEOUT=6`)+ 每次全新连接重试(`_MAX_ATTEMPTS=3`)**——好连接亚秒~数秒完成,撞上卡死的 12s 放弃、重试基本立刻命中好连接;最坏 3×12=36s < 客户端 60s(仍会优雅降级返 200 占位卡)。单文件热补丁 scp 到 ECS + restart,3/3 端到端返真实卡 ~3.3s 验通。**教训:诊断"某外部 API 慢/超时"先分清是生成慢 vs 传输卡死(看 TTFB vs total)、是普遍慢 vs 偶发卡(连打 N 次量失败率),别一上来就加大超时(治标且拖长干等)。**
...
- **URL 构造坑(v1.3.0 致命,异源审才抓到)**:`APIClient.get()/post()` 一律走 `APIClient.makeURL`(`URL(string:path,relativeTo:base)`),**禁用 `baseURL.appendingPathComponent(path)`**——后者把带 `?query` 的整个 path 当**单个路径组件**、`?`→`%3F`,致 `?code=`/`?week=` 端点(`GET /positions/correlation`、`GET /review?week=`)真后端**恒 404**。危险在**被藏死**:`send()` 把 404 映射 `.notHolding`、`checkCorrelation` 按"失败静默"吞掉,且**单测不走真 URL 构造层所以全绿**。新增带 query 的端点务必确认走 makeURL,并加 `testMakeURLPreservesQueryString` 式 URL 门禁(断言 `?` 不被编码成 `%3F`)。
- **macOS `test` destination 测试宿主 quirk(环境级)**:`xcodebuild -destination 'platform=macOS' test` 有时报 `Could not find test host for LinoNTests`(TEST_HOST 解析失败),`.xcodeproj`/`project.yml` 无变化也复现、清 DerivedData/xcodegen 重生都不解。**XCTest 门禁改走 iOS Simulator**(`-destination 'platform=iOS Simulator,name=LinoJ-iPhone16Pro' test`,能跑全部单测);macOS 侧只用 `build` 验证编译。别当回归追。
