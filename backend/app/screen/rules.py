...
def mv_elastic_score(mv_yi: float, cfg: Optional[Dict[str, Any]] = None) -> float:
    ...
    该阈不进配置,仍是 rules 常量单一源);再大恒 0;下行到 mv_floor 及以下线性衰减到 0
    (微盘流动性/操纵风险)。**mv_yi<=0(缺失/未知市值)→ 中性 0.5**(不当微盘惩罚,
    无凭据不误伤,plan §4.1 🔵,与 _normalize 全相等中性一致)。
    ...
    if mv_lo <= mv_yi <= mv_hi:
        return 1.0                                # 中小盘满分带
    if mv_yi > mv_hi:
        # 上行衰减:mv_hi→1, MEGA_CEIL 及以上→0(MEGA_CEIL 不进配置,仍是常量)
...
#
# 键集 = 9 权重(WEIGHTS 原样) + 12 阈值(各自对应上面已存在的 rules 常量)。
# HIGH_EXCLUDE_PCT/HIGH_WARN_PCT/MV_MEGA_CEIL/VOL_MULTIPLE_MIN/NEW_HIGH_DAYS/MA_DAYS/
