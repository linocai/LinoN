...
#     上沿 → 加分,布尔 0/1。
#   · 打分展示(v1.4.1 Phase C1 改绝对口径):`clamp(原始加权分×100, 0, 100)`,跨天可比。
...
# —— v1.4.1 Phase C2:两因子绝对曲线拐点(plan §4.2)——————————————————————
# 经验默认、可复盘迭代、不卡生死。不进 SCREEN_CONFIG_SPEC(本版不扩配置,与
# pos_health/mv 曲线拐点同样不可调一致)。
# 联动提醒:vol_ratio_score 拐点与 VOL_RATIO_MIN(粗筛量比下限)联动——用户若经
# config 把 vol_ratio_min 调到 > VOL_RATIO_SCORE_CAP,则所有 survivors 的量比都
# ≥ VOL_RATIO_SCORE_CAP、vol_ratio_score 全饱和为 1.0,该因子失去区分度
# (已知接受行为,非 bug)。
VOL_RATIO_SCORE_FLOOR = 1.0       # 量比绝对曲线起点(<=1.0 → 0,缩量/平量无意义)
VOL_RATIO_SCORE_CAP = 3.0         # 量比绝对曲线封顶(>=3.0 → 1.0,防天量票畸高)
FUND_RATE_SCORE_FLOOR = 0.0       # 资金占比绝对曲线起点(%,<=0 → 0,净流出/持平不加分)
FUND_RATE_SCORE_CAP = 15.0        # 资金占比绝对曲线封顶(%,>=15 → 1.0,经验默认可迭代)
...
def mv_elastic_score(mv_yi: float, cfg: Optional[Dict[str, Any]] = None) -> float:
    ...
    (缺失/未知市值)→ 中性 0.5**(不当微盘惩罚,无凭据不误伤,plan §4.1 🔵)。
...
# —— v1.4.1 Phase C2:两因子绝对评分曲线(替换 rank_score 里 vol_ratio/fund 的
# min-max 相对归一,plan §4.2)——————————————————————————————————————————————

def vol_ratio_score(vr: float) -> float:
    """量比绝对曲线(官方 daily_basic.volume_ratio)→ [0,1]。

    分段线性:vr<=1.0(平量/缩量,无意义)→ 0;1.0<vr<=3.0 → 线性 0→1;
    vr>3.0 → 1.0(封顶,防天量票畸高)。单调不减。
    """
    if vr <= VOL_RATIO_SCORE_FLOOR:
        return 0.0
    if vr >= VOL_RATIO_SCORE_CAP:
        return 1.0
    span = VOL_RATIO_SCORE_CAP - VOL_RATIO_SCORE_FLOOR
    return (vr - VOL_RATIO_SCORE_FLOOR) / span


def fund_rate_score(rate: float) -> float:
    """资金占比绝对曲线(近3日主力净额占成交额比例合计,%,正=净流入)→ [0,1]。

    分段线性:rate<=0(净流出/持平,不加分)→ 0;0<rate<=15 → 线性 0→1
    (15% 三日累计净流入占比视为强);rate>15 → 1.0(封顶,防个别异常高占比畸高)。
    """
    if rate <= FUND_RATE_SCORE_FLOOR:
        return 0.0
    if rate >= FUND_RATE_SCORE_CAP:
        return 1.0
    span = FUND_RATE_SCORE_CAP - FUND_RATE_SCORE_FLOOR
    return (rate - FUND_RATE_SCORE_FLOOR) / span
...
# RECENT_FLOW_DAYS **不在此键集**(plan §4 config 形状表未列,仍是
# rules.py 常量单一源、不进配置/不可调)。SCORE_FLOOR 已随 v1.4.1 Phase C1 展示分绝对
# 口径改造删除(旧"避免末位恒 0"语义在绝对口径下取消,弱势票诚实显低分是刻意的)。
#
...
def rank_score(
    ...
    """对一批候选算机械排序分(越大越靠前;v1.3.1 九因子,v1.4.1 Phase C2 两因子绝对化)。

    · vol_ratio/fund 走绝对评分曲线(vol_ratio_score/fund_rate_score,plan §4.2),
      **不再 min-max 池内相对归一**——跨天可比、等值输入产等值分。
    ...
    out: List[float] = []
    for i in range(n):
        vol_f = vol_ratio_score(vol_ratios[i])
        fund_f = fund_rate_score(fund_3d[i])
