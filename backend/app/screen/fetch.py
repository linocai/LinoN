...
字段口径(2026-06 真实冒烟校验,见 CLAUDE.md):
  · daily_basic:close / turnover_rate(%) / total_mv(万元) / volume_ratio(量比,
    v1.3.1 A2 起用于粗筛/排序,NaN 安全读取见 _safe_float)。
...
def _safe_float(x, default: float = 0.0) -> float:
    """DataFrame 单元格 → float,NaN 安全(重要#4)。

    Tushare 缺值在 pandas DataFrame 里是 **NaN 非 None**——`float(x or 0.0)` 拦不住
    (NaN 是 truthy,`nan or 0.0` 仍是 nan);必须显式 `pd.isna()` 判断。None/NaN/无法
    转 float → default。用于本 Phase 新读的 volume_ratio 等数值列(v1.3.1 A2)。
    """
    import pandas as pd
    if x is None:
        return default
...
class StockRow:
    ...
    vol_multiple: float = 0.0      # 展示用自算放量倍数(当日量/5日均量,展示口径不变,v1.3.1)
    volume_ratio: float = 0.0      # 官方量比(daily_basic.volume_ratio,v1.3.1 粗筛/排序用此口径)
    ...
    pos_health: float = 0.0        # 位置健康(距60日高点,v1.3.1 A1)
    breakout_ok: bool = False      # 横盘突破(v1.3.1 A1 新增,信号7)
...
def fetch_market_snapshot(
    ...
    for _, r in db.data.iterrows():
        ...
        # 官方量比(v1.3.1 A2):NaN 安全读取(重要#4),缺值/NaN → 0.0(粗筛按 <VOL_RATIO_MIN
        # 淘汰,不会带 NaN 毒化 rank_score 的 min-max)。
...
def _enrich_form(
    ...
    # v1.3.1 A2:sr.volume_ratio 由调用方(fetch_market_snapshot)在调用本函数前已写入
    ...
    sr.pos_health = result.pos_health
    sr.breakout_ok = result.breakout_ok
