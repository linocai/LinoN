...
- 设计已闭合(v2);**阶段 0(基建)已完工**(详见 `archive/stage0_基建_plan.md`);**阶段 1(脊椎+今日台)在施工**——**track A 后端脊椎(A.1–A.5)已完工** + **track B 客户端(B.1–B.4)已完工**(双端 build SUCCEEDED + 本地真跑通,见 §6 [2026-06-21] 两条),**track C(ECS 部署)已完工**(真上 hz ECS:`https://ln.linotsai.top` 健康通、ECS→APNs sandbox 真机推送实测 200、邻居 lf/lw 无恙)。**阶段1 三轨齐,待真机 prod 端到端验收**。
...
- **门禁数字**:已发布 **0** 个阶段;在施工 **1** 个阶段(阶段1,track A+B+C 完工,待真机 prod 端到端验收)。阶段0 验收 = "数据能稳定拉" ✅。**后端 pytest 98 条全绿**(阶段0 的 40 + track A 56 + track B 联调 price 填充 2);**客户端 XCTest 17 条全绿**(签名公式/日历/AppModel)。**双端 build:iOS Simulator + macOS 各 `BUILD SUCCEEDED`,无 body 超时**。
- **track A 已真跑过**(本地):FastAPI 6 端点 `uvicorn` 起、curl 走通 health/401/open→list→close 闭环 + 漏录防护 409/404 + device/ack(`scripts/smoke_api.sh`);APNs JWT 用临时 EC key 单测验签通过(ES256/header/claims)。
- **track B 已真跑过**(本地):iOS 模拟器(LinoJ-iPhone16Pro,iOS 26.5)启动到 TodayView 渲染后端 3 持仓 + 触损红卡(双线呼吸光环/D pips/教练横幅占位文案);macOS 240px 玻璃侧栏 + KPI 横条 + 卡片;`GET /positions 200`(后端供 price)、开/清仓闭环 curl 验(409 满仓/重复、404 重复清仓、stop_line 派生)。**待联调(留 track C/真机)**:真 APNs 实推、真 device token(模拟器拿不到)、锁屏通知卡+动作、ECS 真部署。
- **track C 已真跑过**(真上 hz ECS):`/opt/linon` + nologin `linon` 用户 + systemd **单 unit**(`linon.service` active)+ nginx `ln.linotsai.top` + certbot 证书;`https://ln.linotsai.top/api/v1/health` ok;**ECS→APNs sandbox→iPhone 真机推送实测 status 200**(硬线卡 + 「标记次日清仓」「问教练」动作按钮);部署坑均已解(rsync 冲 setgid 复原、pip 阿里云镜像、`sync.sh` pipefail 守卫 bug、APNs category `LN_HARDLINE`→`HARDLINE` 接缝 bug)。**待真机 prod 端到端**:App 切 prod 环境 + 填 `API_TOKEN` → 注册 device token 到 prod DB → 开仓/监控真推完整闭环。
