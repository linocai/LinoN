...
- 设计已闭合(v2);**阶段 0(基建)已完工**(详见 `archive/stage0_基建_plan.md`);**阶段 1(脊椎+今日台)在施工**——**track A 后端脊椎(A.1–A.5)已完工** + **track B 客户端(B.1–B.4)已完工**(双端 build SUCCEEDED + 本地真跑通,见 §6 [2026-06-21] 两条),**track C(ECS 部署)待开工**。
...
- **门禁数字**:已发布 **0** 个阶段;在施工 **1** 个阶段(阶段1,track A+B 完工 / C 未开工)。阶段0 验收 = "数据能稳定拉" ✅。**后端 pytest 98 条全绿**(阶段0 的 40 + track A 56 + track B 联调 price 填充 2);**客户端 XCTest 17 条全绿**(签名公式/日历/AppModel)。**双端 build:iOS Simulator + macOS 各 `BUILD SUCCEEDED`,无 body 超时**。
