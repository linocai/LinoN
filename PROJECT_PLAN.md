...
- **v1.4 施工中**:Phase A(后端盘中数据层)已完工——新建 `backend/app/data/intraday.py`(4 个纯函数:`_is_intraday_window`/`elapsed_trading_minutes`/`intraday_vol_ratio`/`vwap_of`/`build_intraday_snapshot`),不自拉价不联网。门禁:后端 pytest **450→474 全绿**(新增 24 条,覆盖 plan 验收清单全部 14+ 项,含 VWAP 真实比例假 Quote 反向公式回归门)。**Phase B(coach 盘中上下文注入)+ Phase C(候选盘中确认端点)已完工**:B 改 `app/llm/analyze.py`(`_fetch_form` 顺带吐 `prev5_avg_vol`;`analyze_stock`/`chat_stock` 新增可选 `intraday_quote`/`is_trading`,仅 coach+盘中+有 Quote 才经 `intraday.build_intraday_snapshot` 组装存入 `context["intraday"]`,candidate 模式/窗口外/quote=None 均不组装)+ `app/llm/prompt.py`(两 prompt 函数 `intraday` 键存在才渲染盘中块,标签区分「昨日 EOD 放量倍数」vs「盘中折算量比」,资金段前置 T-1 约束句 + 末尾盘中护栏句,`SYSTEM_PROMPT`/`CHAT_SYSTEM_PROMPT` 各补一句 guardrail)+ `app/api/app.py`(`coach_position`/`chat` 端点层判 `intraday._is_intraday_window(datetime.now())` + 盘中单票 `_quotes_fn` 拉一拍完整 Quote 供 pnl_pct 复用,新增 `_resolve_intraday_quote` helper)。C 新增 `GET /api/v1/candidates/intraday`(读时叠加不落库,复用 `require_token`;批量 `_quotes_fn` 一拍 + 逐票 prev5 均量按 `(code,trade_date)` 进程内缓存 `_PREV5_CACHE`,新增可注入 `_daily_fn`;响应 `{ok,isTrading,tradeDate,asof,degraded,items[]}` 逐字段对齐 plan camelCase 契约,`asof` 取首个非空 quote.ts)。门禁:后端 pytest **474→497 全绿**(Phase B 新增 13、Phase C 新增 10,覆盖两阶段验收清单全部条目;3 处既有测试 fake 签名`_inject_analyze`/`_fake_chat_fn`/`_fake_analyze`补充 `intraday_quote`/`is_trading` 可选参免破坏)。TestClient 端到端抽样验证 `/coach`(盘中注入 Quote 后 reason 文本含真实盘中价)与 `/candidates/intraday`(交易时段返回 5 条 items 各带 chgPct/openChgPct/isAboveVwap/intradayVolRatio/volNote)响应形状均符合契约。**Phase D(客户端双端)已完工**:`Models.swift` 新增 `IntradayItem`(实时字段全 Optional)+ `IntradayConfirmResult`;`APIClient.swift` 给 `get()` 加 `timeout` 可选参(默认 12,重要#3)+ 新增 `fetchCandidatesIntraday()`(30s 长超时,经 makeURL);`AppModel.swift` 新增 `intraday`/`intradayLoading`/`loadIntradayConfirm()`/`intradayItem(byCode:)`(按 code join,建议#10);`CandidatesView.swift` 双端各加「盘中确认」按钮(iOS header / macOS 工具栏,初始可点,响应 `isTrading=false` 才禁用+标注非交易时段,建议#9)+ `CandidateRow` 叠加盘中续强行(现价/涨幅/高开/站VWAP徽章/折算量比,chgPct 数值派生着色不字符串判负)。门禁:客户端 XCTest **95→109 全绿**(新增 14 条:解码前向兼容 4、AppModel 状态机 3、按钮禁用态 4、着色派生 3);双端 `BUILD SUCCEEDED`。**reviewer 审查(0 致命/3 🟡/8 🔵)审后修复已完工**:🟡#1「盘中确认」按钮不再真禁用(仅拉取中禁用;isTrading=false 只视觉变暗+标注非交易时段,可随时再点重查,以最新响应为准,根除永久 brick);🟡#2 prev5 基准窗口改 `vols[:5]`(app.py `_prev5_avg_vol` + analyze.py `_fetch_form`,对齐 plan「取最近 5 条」=T-1..T-5 字面口径,原 `vols[1:6]` 在盘中调用场景系统性偏移一天、抬高折算量比);🟡#3 `chat_stock` 盘中快照组装改直用 `datetime.now()`(不再 `now or datetime.now()` 混用 date/datetime,对齐 `analyze_stock`)。顺手修 5 条 🔵:iOS 叠加行补「高开」字段(对齐 macOS)、非交易时段每行冗余标注去重(整行不渲染,顶部 banner 已够)、无候选时 `isTrading` 改如实回传窗口真值(不再硬编 false,degraded 单独表意)、`_PREV5_CACHE` 键补 `today` 维度防跨天污染、新增 `backend/scripts/smoke_intraday.py`(Phase E 冒烟脚本,交易时段跑两源一致性+VWAP 区间+量能折算数字,非盘中窗口打印退出不报错)。门禁:后端 pytest **497→498 全绿**(新增 2、修订 1);客户端 XCTest **109→115 全绿**(按钮态测试重写 + 新增叠加行/高开镜像测试);双端 `BUILD SUCCEEDED`。**待办**:Phase E 需交易时段手动跑 `smoke_intraday.py` 真复测两源(脚本已就绪,非交易时段验证过优雅退出)。
