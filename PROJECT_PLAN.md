...
- **v1.3.1(盘后选股完善)施工中**:三块——① 新选股逻辑(删高位硬排除改只标注红/琥珀分级、粗筛量比口径、排序 9 因子集含距60日高点/横盘突破、量比接 `daily_basic.volume_ratio`、warnLevel 经 candidates 缓存表往返=**第四次真 migration `warn_level` 列**)、② 选股配置可调化(档 B·App 内调参屏 + 新表 `screen_config` 存 JSON 增量 + `GET/PUT` 端点 + 显式穿参生效,rules 常量降级为默认值/fallback)、③ 候选刷新改纯手动(删 15:35 自动 tick + `last_candidate_date` 防重,回填改挂 EOD 块)。持仓教练深判重做 + 盘中选股独立板块已定移 **v1.4**,本版不碰。§4 有完整 Phase(已过 plan-critic 一轮:1 致命[缓存断层]+6 重要+6 建议全吸收)。
  ...
  - **审后修复(🟡#1+🟡#2+用户新增 mega_ceil 可调化)已完工**:① **mega_ceil 可调化**(用户提出):`MV_MEGA_CEIL` 500→**1500**(原与 `MV_SMALL_CAP_HI`=500 重合致上行衰减带 span=0 退化成硬台阶),升级为 `SCREEN_CONFIG_SPEC` **第 22 键** `mv_mega_ceil`(range[500,5000]),`mv_elastic_score` 从 cfg 读取(cfg=None 回落常量);客户端 `Models.swift` `ScreenConfigSpec.thresholdFields` 同步加第 13 阈值项(range 500...5000、"市值衰减上限(亿)"),`ForEach` 驱动 UI 零改自动多一个步进。验证默认下 500亿=1.0、1000亿≈0.5、1500亿=0、>1500=0。② **带内一致性收口(🟡#1)**:新增 `_enforce_band_consistency`,在逐键夹紧后、归一前对"换手带"(`turnover_lo<turnover_hi`)与"市值带"(`mv_floor<mv_lo<mv_hi<mv_mega_ceil`)做序约束,违反则该带相关键**整组回退默认**(不做局部修补,PUT/resolve 两路径都跑)。③ **保存/恢复默认按钮禁用守卫(🟡#2)**:`ScreenConfigView` 新增 `actionsDisabled`(`screenConfigLoading || screenConfigSaving || screenConfig.isEmpty`)挂两按钮 `.disabled`。后端 pytest **443→450 全绿**(新增 7:mv_mega_ceil 穿参 1 + 带一致性 6);客户端 XCTest **90→95 全绿**(新增 5:mv_mega_ceil 字段 1 + 按钮禁用守卫 4),双端 `BUILD SUCCEEDED`。🟡#3(文档滞后)随本次 §3/§6 更新一并收口。
